...
from .mongo_serializers import FleetLogSerializer
...
def fleet_logs_list_create(request):
    ...
    serializer = FleetLogSerializer(data=request.data)
...
def fleet_logs_detail(request, id: str):
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = FleetLogSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
