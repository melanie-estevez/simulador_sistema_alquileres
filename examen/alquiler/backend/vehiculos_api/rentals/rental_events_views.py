...
from .mongo_serializers import RentalEventSerializer
...
def rental_events_list_create(request):
    ...
    serializer = RentalEventSerializer(data=request.data)
    serializer.is_valid(raise_exception=True)

    data = serializer.validated_data
    data.setdefault("created_at", datetime.now())  # NO se envía desde el cliente, se toma la fecha actual
...
def rental_events_detail(request, id: str):
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = RentalEventSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
