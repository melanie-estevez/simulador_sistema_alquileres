...
from .mongo_serializers import OperationLogSerializer
col = db["operation_logs"]
...
def operation_logs_list_create(request):
...
@permission_classes([IsAuthenticated])
def operation_logs_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = OperationLogSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
