...
from .mongo_serializers import EventTypeSerializer

col = db["system_events"]
...
def system_events_list_create(request):
...
@permission_classes([IsAuthenticated])
def system_events_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = EventTypeSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
